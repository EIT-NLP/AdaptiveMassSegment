...
class KVPressTextGenerationPipeline(Pipeline):
    ...
    def preprocess(
        ...
        if self.tokenizer.chat_template is None:
            ...
            context, question_suffix = context.split(separator, 1)
    ...
    def generate_answer(
        ...
        force_fixed_length = os.environ.get("FORCE_FIXED_LENGTH", "0") == "1"
        ...
        for i in range(max_new_tokens - 1):
            ...
            if not force_fixed_length:
                if new_id.item() in should_stop_token_ids:
                    break

        gen_ids = torch.stack(generated_ids)  # [T_gen]

        # Expose the raw generation for downstream evaluation/debug scripts.
        self.last_generated_ids = gen_ids.detach().cpu()
        self.last_generated_text = self.tokenizer.decode(gen_ids, skip_special_tokens=False)
