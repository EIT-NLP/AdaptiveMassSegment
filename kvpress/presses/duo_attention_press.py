...
class DuoAttentionPress(BasePress):
    ...
    def compression_ratio(self) -> float:
        ...
        if self.compression_ratio_ is None:
            return 0.0
...
def duo_attention_on_the_fly(model, num_samples=50, q_len=500):
    ...
    import pandas as pd
    dataset_path = os.environ.get("DUO_ATTENTION_CALIBRATION_DATA")
    if dataset_path:
        dataset = pd.read_parquet(dataset_path)
    else:
        dataset = load_dataset("HuggingFaceH4/MATH-500", split="test").to_pandas()
    text_column = "question" if "question" in dataset.columns else "problem"
    texts = dataset.sample(min(num_samples, len(dataset)), random_state=42)[text_column].astype(str).tolist()
